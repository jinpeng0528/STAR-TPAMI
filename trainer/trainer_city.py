                ...
                if 'by_class' in met().keys():
                    ...
                    for i in range(len(met()['by_class'])):
                        if i in self.evaluator_val.new_classes_idx:
                            by_class_str = by_class_str + f"{i:2d} *{City[i]} {met()['by_class'][i]:.2f}\n"
...
class Trainer_incremental(Trainer_base):
    ...
    def __init__(
        ...
        if self.save_prototypes:
            prev_info_path = \
                ...
                "/saved_info-epoch{}.pth".format(config['trainer']['epochs'])
        else:
            prev_info_path = \
                str(config.save_dir)[:-len(str(config.save_dir).split('_')[-1])] + \
                str(config['data_loader']['args']['task']['step'] - 1) + \
                "/new_saved_info-epoch{}.pth".format(config['trainer']['epochs'])
        ...
        if 'bitwise_noise' in prev_info.keys():
            self.prev_bitwise_noise = prev_info['bitwise_noise'].to(self.device)
        if 'angle_noise' in prev_info.keys():
            self.prev_angle_noise = prev_info['angle_noise'].to(self.device)
        if 'dist_noise' in prev_info.keys():
            self.prev_dist_noise = prev_info['dist_noise'].to(self.device)
    ...
    def _print_train_info(self):
        ...
        self.logger.info(f"Total loss = {self.config['hyperparameter']['mbce']} * L_mbce + "
                         ...
                         f"{self.config['hyperparameter']['cont']} * L_cont")
            ...
            with torch.cuda.amp.autocast(enabled=self.config['use_amp']):
                ...
                for cls in range(0, self.per_iter_prev_number.shape[0]):
                    ...
                    if self.noise_type == 'angle':
                        random_angle = \
                            torch.randn(per_cls_fake_features.shape[2], device=per_cls_fake_features.device) * self.prev_angle_noise[cls]
                        random_angle = torch.abs(random_angle)
                        random_diff = torch.randn_like(per_cls_fake_features.squeeze(0).squeeze(2))
                        random_diff = F.normalize(random_diff, dim=0)
                        parallel_component = \
                            (random_diff * per_cls_fake_features.squeeze(0).squeeze(2)).sum(0) * \
                            per_cls_fake_features.squeeze(0).squeeze(2)
                        perpendicular_component = random_diff - parallel_component
                        per_cls_fake_features += \
                            (torch.tan(random_angle) * F.normalize(perpendicular_component, dim=1)).unsqueeze(0).unsqueeze(3)
                        per_cls_fake_features = F.normalize(per_cls_fake_features, dim=1)

                    elif self.noise_type == 'bitwise':
                        noise = \
                            torch.randn_like(per_cls_fake_features) * self.prev_bitwise_noise[cls].reshape(1, -1, 1, 1)
                        per_cls_fake_features = per_cls_fake_features + noise
                        per_cls_fake_features = F.normalize(per_cls_fake_features, dim=1)

                    elif self.noise_type == 'dist':
                        random_dist = torch.randn(per_cls_fake_features.shape[2], device=per_cls_fake_features.device) \
                                      * self.prev_dist_noise[cls]
                        random_dist = torch.abs(random_dist).clamp(max=1.999)
                        theta = 2 * torch.asin(random_dist / 2)

                        random_diff = torch.rand_like(per_cls_fake_features.squeeze(0).squeeze(2)).T - 0.5
                        proj = torch.matmul(random_diff, self.prev_prototypes[cls]).unsqueeze(1) \
                               * self.prev_prototypes[cls]
                        random_diff_orthogonal = random_diff - proj
                        random_diff_orthogonal = F.normalize(random_diff_orthogonal, dim=1)
                        random_diff_orthogonal = random_diff_orthogonal * torch.tan(theta).unsqueeze(1)

                        per_cls_fake_features = \
                            per_cls_fake_features + random_diff_orthogonal.T.unsqueeze(0).unsqueeze(3)
                        per_cls_fake_features = F.normalize(per_cls_fake_features, dim=1)
                ...
                logit, features, extra = \
                    self.model(data['image'], ret_intermediate=True, fake_features=fake_features,
                               region_bg=region_bg, model_add_norm=True)
