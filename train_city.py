...
from trainer.trainer_city import Trainer_base, Trainer_incremental
...
def main_worker(gpu, ngpus_per_node, config):
    ...
    if (task_step > 0) and (config['data_loader']['args']['memory']['mem_size'] > 0):
        memory_sampling_balanced(
            ...
            ('city', task_setting, task_name, task_step),
            logger, gpu,
        )
    ...
    if task_step > 0:
        ...
        #     'optimizer',
        #     torch.optim,
        #     [{"params": model.get_backbone_params(), "lr": config["optimizer"]["args"]["lr"] * 0},
        #      {"params": model.get_aspp_params(), "lr": config["optimizer"]["args"]["lr"] * 0},
        #      {"params": model.get_old_classifer_params(), "lr": config["optimizer"]["args"]["lr"] * 10},
        #      {"params": model.get_new_classifer_params(), "lr": config["optimizer"]["args"]["lr"] * 100}]
        # )
    else:
...
if __name__ == '__main__':
    ...
    options = [
        ...
        CustomArgs(['--save_prototypes'], action='store_true', target='trainer;save_prototypes'),
        CustomArgs(['--save_norms'], action='store_true', target='trainer;save_norms'),
