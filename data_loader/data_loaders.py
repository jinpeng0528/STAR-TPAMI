...
from data_loader.dataset import (
    VOCSegmentationIncremental, ADESegmentationIncremental, CitySegmentationIncremental,
    VOCSegmentationIncrementalMemory, ADESegmentationIncrementalMemory, CitySegmentationIncrementalMemory
)
    ...
    def __init__(self, task, train, val, test, num_workers, pin_memory, memory=None):
        ...
        if self.step > 0 and (memory is not None) and memory['mem_size'] != 0:
            ...
            self.prev_train_set = VOCSegmentationIncremental(
                ...
                idxs_path=Path(
                    task['idxs_path']) / "voc" / f"{task['setting']}_{task['name']}_train_{self.step - 1:02d}.npy",
                **train['args'],
            )
...
class CityIncrementalDataLoader():
    def __init__(self, task, train, val, test, num_workers, pin_memory, memory=None):
        self.task = task
        self.train = train
        ...
        self.step = task['step']
        self.name = task['name']
        self.classes_idx_new, self.classes_idx_old = get_task_labels('city', self.name, self.step)
        self.setting = task['setting']
        self.n_classes = len(list(set(self.classes_idx_new + self.classes_idx_old)))

        self.train_set = CitySegmentationIncremental(
            setting=self.setting,
            classes_idx_new=self.classes_idx_new,
            classes_idx_old=self.classes_idx_old,
            idxs_path=Path(task['idxs_path']) / "city" / f"{task['setting']}_{task['name']}_train_{self.step:02d}.npy",
            **train['args'],
        )
        ...
        if val['cross_val'] is True:
            ...
            # Validatoin using validation set.
            self.val_set = CitySegmentationIncremental(
                val=True,
                setting=self.setting,
                classes_idx_new=self.classes_idx_new,
                classes_idx_old=self.classes_idx_old,
                idxs_path=Path(
                    task['idxs_path']) / "city" / f"{task['setting']}_{task['name']}_val_{self.step:02d}.npy",
                **val['args'],
            )
        self.test_set = CitySegmentationIncremental(
            test=True,
            setting=self.setting,
            classes_idx_new=self.classes_idx_new,
            classes_idx_old=self.classes_idx_old,
            idxs_path=Path(task['idxs_path']) / "city" / f"{task['setting']}_{task['name']}_test_{self.step:02d}.npy",
            **test['args'],
        )

        self.memory = None
        if self.step > 0 and (memory is not None) and memory['mem_size'] != 0:
            classes_idx_new, classes_idx_old = get_task_labels('city', self.name, self.step - 1)
            self.prev_train_set = CitySegmentationIncremental(
                setting=self.setting,
                classes_idx_new=classes_idx_new,
                classes_idx_old=classes_idx_old,
                idxs_path=Path(
                    task['idxs_path']) / "city" / f"{task['setting']}_{task['name']}_train_{self.step - 1:02d}.npy",
                **train['args'],
            )
    ...
    def get_memory(self, config, concat=True):
        if self.step > 0:
            self.memory = CitySegmentationIncrementalMemory(
                setting=self.setting,
                step=self.step,
                classes_idx_new=self.classes_idx_new,
                classes_idx_old=self.classes_idx_old,
                # idxs_path=Path(self.task['idxs_path']) / "city" / f"{self.task['setting']}_{self.task['name']}_memory.json",
                idxs_path=config.save_dir.parent / f'step_{self.step}' / 'memory.json',
                **self.train['args'],
            )
            if concat is True:
    ...
    def get_per_task_classes(self, step=None):
        if step is None:
            step = self.step
        return get_per_task_classes('city', self.name, step)

    def get_task_labels(self, step=None):
        if step is None:
            step = self.step
        return get_task_labels('city', self.name, step)