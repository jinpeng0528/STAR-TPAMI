...
class DeepLabV3(nn.Module):
    ...
    def forward(self, x, ret_intermediate=False, fake_features=None, region_bg=None, model_add_norm=False):
        ...
        if fake_features is not None:
            if model_add_norm:
                logits_for_fake = self.forward_class_prediction(fake_features * torch.norm(x_pl, dim=1).mean().item())
            else:
